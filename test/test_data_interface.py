...
def sut():
    ...
    with patch('sqlite3.connect'):
        with patch('pandas.read_sql_query'):
            with patch('geopandas.points_from_xy'):
                with patch('geopandas.GeoDataFrame'):
                    with patch('utils.collection_utils.join_with_comma'):
                        return DataInterface(id_scenario=1, scenario_options=scenario_options, id_region=10)
...
def test__read_site_data(sut):
    ...
    with patch('pandas.read_sql_query') as patched_read_sql_query:
        with patch('geopandas.points_from_xy') as patched_points:
            with patch('geopandas.GeoDataFrame') as patched_geo_data_frame:
