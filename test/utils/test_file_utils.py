...
def test_create_folder_if_not_exists():
    with patch('os.path.isdir', return_value=False) as patched_isdir:
        with patch('os.mkdir') as patched_mkdir:
...
def test_delete_file_if_exists():
    with patch('os.path.isfile', return_value=True) as patched_isfile:
