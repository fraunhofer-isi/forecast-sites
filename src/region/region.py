...
class Region(Entity):
    ...
    def site_df(self):
        site_df = self._data_interface.site_data.copy(True)
        shorter_column_names = {
            'id_company': 'id_comp',
            'co2_equivalent_2015_in_tons': 'co2_equ',
        }
        site_df.rename(columns=shorter_column_names, inplace=True)
        return site_df
    ...
    def get_process_ids_for_site(self, id_site):
        site = list(filter(lambda current_site: current_site.id == id_site, self.sites))[0]
        process_ids = []
